...
def cycle_length(g, cycle):
    ...
    for i in edges:
        paths.update({i : g.get_edge_data(*i)})

    for j in paths:
        updatedPaths.update({j : paths[j]['weight']})
    
    for i in path:
        for j in updatedPaths:
            if(j[0] == i[0] and j[1] == i[1] or j[0] == i[1] and j[1] == i[0]):
                weight += updatedPaths[j]
    return weight
